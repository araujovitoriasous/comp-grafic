...
// Variáveis para controlar o tamanho fixo do desenho
const float FIXED_SIZE = 0.8f;  // Tamanho fixo do desenho (em coordenadas OpenGL)
...
void reshape(int w, int h) {
    // Define a área de desenho (viewport) para toda a janela
    ...
    // Define projeção ortogonal que mantém o desenho com tamanho fixo
    // O desenho sempre terá o mesmo tamanho em pixels, independente do redimensionamento
    if (w <= h) {
        // Largura menor - ajusta altura para manter proporção
        glOrtho(-1.0, 1.0, -1.0/aspect, 1.0/aspect, -1.0, 1.0);
    } else {
        // Altura menor - ajusta largura para manter proporção
    ...
    // Aplica transformação para posicionar o desenho no canto inferior esquerdo
    // e manter tamanho constante
    glTranslatef(-1.0f + FIXED_SIZE/2, -1.0f + FIXED_SIZE/2, 0.0f);  // Move para canto inferior esquerdo
    glScalef(FIXED_SIZE, FIXED_SIZE, 1.0f);  // Aplica escala fixa
}
...
int main(int argc, char** argv) {
    ...
    glutCreateWindow("Sol Amarelo - Tamanho Fixo");
